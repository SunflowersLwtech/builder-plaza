...
| Demo URL | `https://github.com/SunflowersLwtech/sunkit` |
...
| 2:55 | B | Home → **edit** intent →<br>**Intent** Seeking co-founder<br>**Note** `Looking for a Flutter partner on Builder Plaza`<br>→ **Save intent** | "GitHub says what I've built. LinkedIn says who I am. Neither of them says whether I'm actually free to start something this month — so that's the third signal we add. I'll set mine to *seeking co-founder*, and as I save, `intents` on the right ticks up." |
| 3:10 | B | ＋ **New project**, then:<br>**Title** `Builder Plaza mobile app`<br>**Stage** BUILDING<br>**Needs** `A Flutter developer to pair on the matching engine`<br>**Demo URL** `https://github.com/SunflowersLwtech/sunkit`<br>**Team division** `I own backend and mobile. Looking to split frontend`<br>**Repo** `SunflowersLwtech/sunkit` → **+ ADD** | "Now, intent on its own is just availability. What a collaborator actually needs is something concrete to judge — so let me post a project. Title, what stage it's at, who I'm looking for, how the work splits. And this one matters: I link a real repository." |
